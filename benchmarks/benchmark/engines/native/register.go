...
func newRegister(db *sql.DB) *Register {
	r := &Register{}
	r.getStmt = util.Try(db.Prepare("select value from native_register where id = $1"))
	r.setStmt = util.Try(db.Prepare("update native_register set value = $2 where id = $1"))
	return r
}
